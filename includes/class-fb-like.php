...
if( !class_exists( 'KIA_FBSP_Like_Widget' ) )
{
	class KIA_FBSP_Like_Widget extends WP_Widget
	{
		...
		function widget( $args, $instance )
		{
			...
			$width = empty( $instance['width'] ) ? ' data-width="292"' : ' data-width="' . $instance['width'] . '"';
			$height = empty( $instance['height'] ) ? ' ' : ' data-height="' . $instance['height'] . '"';
			$url = empty( $instance['url'] ) ? ' data-href="http://www.facebook.com/WordPress"' : ' data-href="' . $instance['url'] . '"';
			...
			$stream = $instance['show_stream'] == 'on' ? ' data-stream="true"' : ' data-stream="false"';
			...
			echo '<div class="fb-like-box"'  . $url . $width . $height . $border . $color . $faces . $stream . $header . '></div>';
			echo $after_widget;
		}
	} // end class
