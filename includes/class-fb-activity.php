...
if( !class_exists( 'KIA_FBSP_Activity_Widget' ) )
{
	class KIA_FBSP_Activity_Widget extends WP_Widget
	{
		function KIA_FBSP_Activity_Widget()
		{
			$widget_ops = array(
			...
			$this->WP_Widget( 'KIA_FBSP_Activity_Widget', __( 'Facebook Activity Feed', 'cd-fbspw' ), $widget_ops );
		}
		...
		function widget( $args, $instance )
		{
			...
			$recommendations = $instance['show_recommendations'] == 'on' ? ' data-recommendations="true"' : ' data-recommendations="false"';
			...
			echo '<div class="fb-activity"' . $app_id . $url . $width . $height . $border . $color . $font . $header . $recommendations . '></div>';
			echo $after_widget;
		}
	} // end class
